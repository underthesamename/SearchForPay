...
function searchPayload(totalAmountCents) {
  return {
    results: totalAmountCents === null ? [] : [{
      ...
      totalCost: { amountCents: totalAmountCents, currency: 'BRL' }
    }],
    meta: {
      providerReports: [{
        providerName: 'marketplace-real',
...
test('alerta salva erro claro quando nenhum provedor real esta disponivel', async () => {
  ...
  assert.match(checked.lastResult.message, /Nenhum provedor real/);
});
...
test('runDueAlerts reconsulta apenas alertas vencidos', async () => {
  ...
  const service = createPriceAlertService({
    store: createMemoryStore(),
    defaultIntervalMs: 60_000,
    now: fixedClock('2026-07-18T10:00:00.000Z', '2026-07-18T10:00:00.000Z'),
    searchService: {
      async search() {
        calls += 1;
  ...
  assert.equal(result.alerts[0].lastResult.status, 'target_not_met');
});
